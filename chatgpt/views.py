...
def chating(request):
    ...
    return render(request, 'gpt/chating.html', {'conversation': conversation})
@login_required
def reset(request):
    global i
    ...
    i = 0
    return redirect('chatgpt:chating')
...
def signup_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            return redirect('http://18.144.167.189/')
    else:
...
def chat_history_view(request):
    chat_histories = ChatHistory.objects.all().order_by('-datetime')
    return render(request, 'gpt/chat_history.html', {'chat_histories': chat_histories})